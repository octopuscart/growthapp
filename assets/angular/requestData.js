...
Admin.controller('BibleStudyDataController', function ($scope, $http, $timeout, $interval) {
    $scope.resultData = {"url": gblurl, "status": "0", "list": [], "bsList": []};

    $scope.selecteIndex = function () {

        $http.get(gblurl).then(function (rdata) {
            $scope.resultData.bsList = rdata.data;
        }, function () {
